...
void CargarRecursos()
{
    ...
    // Cargamos la nueva para el centro (Asegúrate de que el archivo se llame ISLA_1.bmp)
    hBmpIslaCentral = CargarImagen("assets/mundo/ISLA_1.bmp");

    // Movemos la vieja a la variable "Noreste"
    hBmpIslaNoreste = CargarImagen("assets/mundo/isla_principal.bmp"); // O ISLA_Primero.bmp
...
void LiberarRecursos()
{
    ...
    for (int i = 0; i < 4; i++)
        for (int j = 0; j < 3; j++)
            if (hBmpJugadorAnim[i][j])
                DeleteObject(hBmpJugadorAnim[i][j]);
    if (hBmpJugador)
    ...
    if (hBmpArbol)
        DeleteObject(hBmpArbol);
    if (hBmpRoca)
        DeleteObject(hBmpRoca);
}
